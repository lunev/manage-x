...
- [x] Feature #17 – Respect `prefers-reduced-motion` for `.fade-in`
...
- [ ] ~~Finding #17 – Expand Options page (backup panel, reset-to-default — theme toggle descoped via Finding #16; shortcut info reverted, see Finding #15)~~
  - Status: Skipped (2026-08-03) — explicitly descoped by the user; reset-to-default extracted to stand-alone Feature #16, backup panel remains out of scope for now | Priority: Low-Medium | Effort: Medium | Dependencies: Finding #8
...
- [ ] ~~Finding #12 – Replace bespoke delete countdown with a real `Dialog` confirmation~~
  - Status: Skipped (2026-08-03) — explicitly descoped by the user; the popup has no room for a modal Dialog surface (see the standing "no Modal Dialog in the extension popup" constraint added to the implementer/architect/reviewer agents) | Priority: Low | Effort: Medium | Dependencies: None
- [ ] ~~Feature #6 – Post-delete "Undo" toast (complements/can replace Finding #12's approach)~~
  - Status: Skipped (2026-08-03) — explicitly descoped by the user, deferred alongside Finding #12 | Priority: Low | Effort: Small | Dependencies: Finding #12
- [ ] ~~Feature #16 – Surface "reset extension to default state" as a user-facing action~~
  - Status: Skipped (2026-08-03) — implemented in v2.0.26, then reverted and descoped by explicit user decision before shipping; not planned | Priority: Low | Effort: Small | Dependencies: None (idea extracted from Finding #17, implemented standalone)
